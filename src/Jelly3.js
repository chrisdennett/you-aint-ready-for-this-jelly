...
const Jelly3 = () => {
  ...
  const [trail] = useTrail(5, () => ({
    from: { y: 0 },
    to: async next => {
      while (1) await next({ y: 1 });
    },
    config: i =>
      i === 0
        ? { duration: 2000 }
        : { duration: 2000, easing: d3.easeSinInOut },
    reset: true
  }));
      ...
      <svg width="100%" height="100%">
        ...
        {trail.map(({ y }, index) => {
          const translation2 = y.interpolate({
            range: [0, 0.2, 0.4, 0.7, 1],
            output: [
              `translate( ${start4Pos.x} ${start4Pos.y + index * 10} )`,
              `translate( ${start3Pos.x} ${start3Pos.y + index * 10} } )`,
              `translate( ${startPos.x} ${startPos.y + index * 10} } )`,
              `translate( ${start2Pos.x} ${start2Pos.y + index * 10} } )`,
              `translate( ${start4Pos.x} ${start4Pos.y + index * 10} } )`
            ]
          });
          return (
            <animated.g key={index} transform={translation2}>
              ...
              <circle cx={200} cy={0} r={nodeRadius} fill={"red"} />
            </animated.g>
          );
        })}
      </svg>
